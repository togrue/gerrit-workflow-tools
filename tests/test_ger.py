...
def test_ger_inbox_help_delegates(stack_repo: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    code, out, _err = _run_ger(stack_repo, ["inbox", "--help"], monkeypatch, catch_sys_exit=True)
    assert code == 0
