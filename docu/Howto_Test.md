...
Install the `gerrit-workflow-tools` package from this workspace (invoked with `PYTHONPATH=<this-repo>/src python -m gerrit_workflow_tools.cli_log` for `ger log`, or another `cli_<subcommand>` module, so the current working directory stays the test repo).
